...
router = APIRouter(tags=["Vector Office"])


@router.post("/vector-office")
@handle_route_exceptions("Failed to process vector office pipeline")
async def trigger_vector_office_pipeline(
    events: Union[List[Dict[str, Any]], Dict[str, Any]]
):
    """Trigger Vector Office pipeline by receiving events directly in body.
    
    This endpoint handles logs from office cameras and Jetson devices.
    """
    ...
    return success_response(
        message="Vector office pipeline processed",
        total_events=len(events),
